...
user_item = load('bookmark_tags.dat');

% user_friends = load(strcat(data_name,'user_friends.dat')');
% fileID = fopen(strcat(data_name,'artists.dat'),'r');
% tline = fgetl(fileID);
% i = 1;
% while ischar(tline)
%     temp = strsplit(tline,'\t');
%     artists(i,1) = str2num(temp{1});
%     tline = fgetl(fileID);
%     i = i+1;
% end
% fclose(fileID);
